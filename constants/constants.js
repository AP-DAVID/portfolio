export const projects = [
  {
    title: 'Musco(Music Ecosystem)',
    description: "Musco provides the user with an ecosystem of sections providing services and functionalities It includes: Five sections(user, music artist, videographers, record labels) interacting an providing services to one another. An inbuilt chat application that makes the ecosystem even closer",
      image: '/images/musco.png',
      tags: ['Mongo', 'Express', 'React', 'Node'],
    source: 'https://muscotest5a.vercel.app',
    visit: 'https://github.com/AP-DAVID/music-ecosystem',
    id: 0,
  },
  {
    title: 'Mood Cognizant Recommender System',
    description:"MCRS takes the user's image for emotion detection then recommends a song(based on a genre) that suits the emotion.",
    image: '/images/images.jfif',
    tags: ['React Native', 'JavaScript'],
    visit: 'https://github.com/AP-DAVID/MCRS',
    id: 1,
  },
  {
    title: 'West Africa Voting Application',
    description: "Genuine, inclusive, free and fair elections are an indispensable component of democratic governance, rule of law and respect for human rights alike, This Application will do just that",
      image: '/images/voting.png',
      tags: ['React', 'WebRTC'],
    source: 'https://westafric.vercel.app',
    id: 2,
  },
  {
    title: 'Game X',
    description: "A game tournament",
    image: '/images/exher.png',
    tags: ['React', 'Express', 'Node', 'MongoDB'],
    source: 'https://gamex.vercel.app',
    id: 3,
  },
];

export const TimeLineData = [
  { year: 2017, text: 'Started my journey', },
  { year: 2018, text: 'Co-founded Blyte tech', },
  { year: 2020, text: 'IT tech team of Ecobank plc', },
  { year: 2021, text: "Got my bachelor's degree", },
 
];